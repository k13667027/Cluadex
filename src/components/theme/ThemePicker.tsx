...
import { getCustomThemeDefinitions } from '../../utils/theme.js';
...
export function ThemePicker(t0: ThemePickerProps) {
  ...
  if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
    const builtinOptions = [...(feature("AUTO_THEME") ? [{
      ...
      label: "── Visual Themes ──",
      value: "__separator1__" as ThemeSetting
    }, {
      label: "Sunset (warm orange)",
      value: "sunset"
    }, {
      ...
      label: "Neon (pink & blue)",
      value: "neon"
    }, {
      label: "Mono (greyscale)",
      value: "mono"
    }, {
      label: "Matrix (green terminal)",
      value: "matrix"
    }];
    const customOptions = getCustomThemeDefinitions().map(def => ({
      label: def.label + (def.description ? ` — ${def.description}` : ''),
      value: def.name,
    }));
    t10 = customOptions.length > 0
      ? [...builtinOptions, { label: '── Custom Themes ──', value: '__separator__' as ThemeSetting }, ...customOptions]
      : builtinOptions;
    $[7] = t10;
  } else {
    t10 = $[7];
  ...
  if ($[18] !== onThemeSelect || $[19] !== savePreview) {
    t16 = (setting_0: string) => {
      if (setting_0.startsWith('__separator')) {
        return;
      }
      savePreview();
  ...
  let t21;
  if ($[35] !== columns) {
    t21 = <Box flexDirection="column" borderTop={true} borderBottom={true} borderLeft={false} borderRight={false} borderStyle="dashed" borderColor="gray">{React.createElement(StructuredDiff as React.ComponentType<Record<string, unknown>>, {patch: t20, dim: false, filePath: "demo.js", firstLine: null, width: columns})}</Box>;
    $[35] = columns;
    $[36] = t21;
  } else {
    t21 = $[36];
...
function _temp2() {}
function _temp(s: { settings: { syntaxHighlightingDisabled?: boolean } }) {
