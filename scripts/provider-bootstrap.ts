...
async function main(): Promise<void> {
  ...
  const goal = normalizeRecommendationGoal(
    parseArg('--goal') || process.env.CLAUDEX_PROFILE_GOAL,
  )
  ...
  if (selected === 'gemini') {
    ...
    const { buildNvidiaProfileEnv } = await import('../src/utils/nvidiaProvider.ts')
    ...
    if (!builtEnv) {
      ...
      console.error('Get a key at: https://build.nvidia.com/')
      process.exit(1)
    }
